...
DECLARE_STATS_GROUP(TEXT("UltraleapTracking"), STATGROUP_UltraleapTracking,
                    STATCAT_Advanced);
DECLARE_CYCLE_STAT(TEXT("Leap Game Input and Events"), STAT_LeapInputTick,
                   STATGROUP_UltraleapTracking);
DECLARE_CYCLE_STAT(TEXT("Leap BodyState Tick"), STAT_LeapBodyStateTick,
                   STATGROUP_UltraleapTracking);

#pragma region Utility
bool FUltraleapTrackingInputDevice::bUseNewTrackingModeAPI = true;
// Function call Utility
void FUltraleapTrackingInputDevice::CallFunctionOnComponents(
    TFunction<void(ULeapComponent *)> InFunction) {
...
bool FUltraleapTrackingInputDevice::EmitKeyUpEventForKey(FKey Key,
                                                         int32 User = 0,
                                                         bool Repeat = false) {
  if (IsInGameThread()) {
    FKeyEvent KeyEvent(Key, FSlateApplication::Get().GetModifierKeys(), User,
                       Repeat, 0, 0);
...
bool FUltraleapTrackingInputDevice::EmitKeyDownEventForKey(
    FKey Key, int32 User = 0, bool Repeat = false) {
  if (IsInGameThread()) {
    FKeyEvent KeyEvent(Key, FSlateApplication::Get().GetModifierKeys(), User,
                       Repeat, 0, 0);
...
bool FUltraleapTrackingInputDevice::EmitAnalogInputEventForKey(
    FKey Key, float Value, int32 User = 0, bool Repeat = false) {
  if (IsInGameThread()) {
    FAnalogInputEvent AnalogInputEvent(
        Key, FSlateApplication::Get().GetModifierKeys(), User, Repeat, 0, 0,
        Value);
...
void FUltraleapTrackingInputDevice::OnConnectionLost() {
  FLeapAsync::RunShortLambdaOnGameThread([&] {
    UE_LOG(UltraleapTrackingLog, Warning,
           TEXT("LeapService: OnConnectionLost."));
...
void FUltraleapTrackingInputDevice::OnDeviceFound(
    const LEAP_DEVICE_INFO *Props) {
  Stats.DeviceInfo.SetFromLeapDevice((_LEAP_DEVICE_INFO *)Props);
  SetOptions(Options);

  LeapImageHandler->Reset();

  UE_LOG(UltraleapTrackingLog, Log, TEXT("OnDeviceFound %s %s."),
         *Stats.DeviceInfo.PID, *Stats.DeviceInfo.Serial);
...
void FUltraleapTrackingInputDevice::OnDeviceLost(const char *Serial) {
  ...
  FLeapAsync::RunShortLambdaOnGameThread([&, SerialString] {
    UE_LOG(UltraleapTrackingLog, Warning, TEXT("OnDeviceLost %s."),
           *SerialString);
...
void FUltraleapTrackingInputDevice::OnDeviceFailure(
    const eLeapDeviceStatus FailureCode, const LEAP_DEVICE FailedDevice) {
  FString ErrorString;
  switch (FailureCode) {
  ...
  UE_LOG(UltraleapTrackingLog, Warning, TEXT("OnDeviceFailure: %s (%d)"),
         *ErrorString, (int32)FailureCode);
}
...
void FUltraleapTrackingInputDevice::OnImage(
    const LEAP_IMAGE_EVENT *ImageEvent) {
  // Forward it to the handler
  LeapImageHandler->OnImage(ImageEvent);
}

void FUltraleapTrackingInputDevice::OnImageCallback(
    UTexture2D *LeftCapturedTexture, UTexture2D *RightCapturedTexture) {
  // Handler has returned with batched easy-to-parse results, forward callback
  // on game thread
  CallFunctionOnComponents(
      [LeftCapturedTexture, RightCapturedTexture](ULeapComponent *Component) {
        Component->OnImageEvent.Broadcast(LeftCapturedTexture,
                                          ELeapImageType::LEAP_IMAGE_LEFT);
        Component->OnImageEvent.Broadcast(RightCapturedTexture,
                                          ELeapImageType::LEAP_IMAGE_RIGHT);
      });
}
...
void FUltraleapTrackingInputDevice::OnTrackingMode(
    const eLeapTrackingMode CurrentMode) {
  switch (CurrentMode) {
...
void FUltraleapTrackingInputDevice::OnLog(const eLeapLogSeverity Severity,
...
FUltraleapTrackingInputDevice::FUltraleapTrackingInputDevice(
  ...
  // Add IM keys
  EKeys::AddKey(FKeyDetails(EKeysLeap::LeapPinchL,
                            LOCTEXT("LeapPinchL", "Leap (L) Pinch"),
                            FKeyDetails::GamepadKey));
  EKeys::AddKey(FKeyDetails(EKeysLeap::LeapGrabL,
                            LOCTEXT("LeapGrabL", "Leap (L) Grab"),
                            FKeyDetails::GamepadKey));
  EKeys::AddKey(FKeyDetails(EKeysLeap::LeapPinchR,
                            LOCTEXT("LeapPinchR", "Leap (R) Pinch"),
                            FKeyDetails::GamepadKey));
  EKeys::AddKey(FKeyDetails(EKeysLeap::LeapGrabR,
                            LOCTEXT("LeapGrabR", "Leap (R) Grab"),
                            FKeyDetails::GamepadKey));
  ...
  LiveLink->Startup();
  LiveLink->SyncSubjectToSkeleton(
      IBodyState::Get().SkeletonForDevice(BodyStateDeviceId));
#endif

  // Image support
  LeapImageHandler = MakeShareable(new FLeapImage);
  LeapImageHandler->OnImageCallback.AddRaw(
      this, &FUltraleapTrackingInputDevice::OnImageCallback);
}
...
void FUltraleapTrackingInputDevice::CaptureAndEvaluateInput() {
  ...
  FingerInterpolationTimeOffset =
      Options.FingerInterpFactor * FrameTimeInMicros;

  if (Options.bUseInterpolation) {
    // Let's interpolate the frame using leap function

    // Get the future interpolated finger frame
    Frame = Leap.GetInterpolatedFrameAtTime(LeapTimeNow +
                                            FingerInterpolationTimeOffset);
    ...
    // lower latency
    Frame = Leap.GetInterpolatedFrameAtTime(LeapTimeNow +
                                            HandInterpolationTimeOffset);
    CurrentFrame.SetInterpolationPartialFromLeapFrame(Frame);

    // Track our extrapolation time in stats
    Stats.FrameExtrapolationInMS =
        (CurrentFrame.TimeStamp - TimeWarpTimeStamp) / 1000.f;
  } else {
    CurrentFrame.SetFromLeapFrame(Frame);
  ...
  if (Options.Mode == LEAP_MODE_VR && Options.bTransformOriginToHMD) {
    ...
    if (Options.bUseTimeWarp) {
      ...
      BodyStateHMDSnapshot SnapshotThen =
          SnapshotHandler.HMDSampleClosestToTimestamp(SnapshotNow.Timestamp -
                                                      Options.TimewarpOffset);

      BodyStateHMDSnapshot SnapshotDifference =
          SnapshotNow.Difference(SnapshotThen);

      FRotator WarpRotation =
          SnapshotDifference.Orientation.Rotator() * Options.TimewarpFactor;
      FVector WarpTranslation =
          SnapshotDifference.Position * Options.TimewarpFactor;

      FinalHMDTranslation += WarpTranslation;

      FinalHMDRotation =
          FLeapUtility::CombineRotators(WarpRotation, FinalHMDRotation);
...
void FUltraleapTrackingInputDevice::CheckHandVisibility() {
  if (UseTimeBasedVisibilityCheck) {
    ...
    if (IsLeftVisible) {
      TimeSinceLastLeftVisible =
          TimeSinceLastLeftVisible + (LeapGetNow() - LastLeapTime);
    }
    if (IsRightVisible) {
      TimeSinceLastRightVisible =
          TimeSinceLastRightVisible + (LeapGetNow() - LastLeapTime);
    }
      ...
      if (Hand.HandType == EHandType::LEAP_HAND_LEFT) {
        if (CurrentFrame.LeftHandVisible) {
          ...
          if (!IsLeftVisible) {
            ...
            CallFunctionOnComponents(
                [this, LeftVisible](ULeapComponent *Component) {
        ...
        if (CurrentFrame.RightHandVisible) {
          ...
          if (!IsRightVisible) {
            ...
            CallFunctionOnComponents([this,
                                      RightVisible](ULeapComponent *Component) {
...
void FUltraleapTrackingInputDevice::CheckPinchGesture() {
  if (UseTimeBasedGestureCheck) {
    if (IsLeftPinching) {
      TimeSinceLastLeftPinch =
          TimeSinceLastLeftPinch + (LeapGetNow() - LastLeapTime);
    }
    if (IsRightPinching) {
      TimeSinceLastRightPinch =
          TimeSinceLastRightPinch + (LeapGetNow() - LastLeapTime);
    }
    for (auto &Hand : CurrentFrame.Hands) {
      const FLeapHandData &FinalHandData = Hand;
      if (Hand.HandType == EHandType::LEAP_HAND_LEFT) {
        if (!IsLeftGrabbing && (!IsLeftPinching &&
                                (Hand.PinchStrength > StartPinchThreshold)) ||
          ...
          if (!IsLeftPinching) {
            ...
            CallFunctionOnComponents(
                [FinalHandData](ULeapComponent *Component) {
        ...
        if (!IsRightGrabbing && (!IsRightPinching &&
                                 (Hand.PinchStrength > StartPinchThreshold)) ||
            ...
            CallFunctionOnComponents(
                [FinalHandData](ULeapComponent *Component) {
                  Component->OnHandPinched.Broadcast(FinalHandData);
                   ...
                   (TimeSinceLastRightPinch > PinchTimeout)) {
    ...
    for (auto &Hand : CurrentFrame.Hands) {
      ...
      const FLeapHandData &FinalHandData = Hand;
      // Pinch
      if (Hand.PinchStrength > StartPinchThreshold &&
          PastHand.PinchStrength <= StartPinchThreshold) {
        if (Hand.HandType == EHandType::LEAP_HAND_LEFT) {
          EmitKeyDownEventForKey(EKeysLeap::LeapPinchL);
        } else if (Hand.HandType == EHandType::LEAP_HAND_RIGHT) {
          EmitKeyDownEventForKey(EKeysLeap::LeapPinchR);
      ...
      else if (Hand.PinchStrength <= EndPinchThreshold &&
               PastHand.PinchStrength > EndPinchThreshold) {
        if (Hand.HandType == EHandType::LEAP_HAND_LEFT) {
          EmitKeyUpEventForKey(EKeysLeap::LeapPinchL);
        } else if (Hand.HandType == EHandType::LEAP_HAND_RIGHT) {
          EmitKeyUpEventForKey(EKeysLeap::LeapPinchR);
...
void FUltraleapTrackingInputDevice::CheckGrabGesture() {
  if (UseTimeBasedGestureCheck) {
    if (IsLeftGrabbing) {
      TimeSinceLastLeftGrab =
          TimeSinceLastLeftGrab + (LeapGetNow() - LastLeapTime);
    }
    if (IsRightGrabbing) {
      TimeSinceLastRightGrab =
          TimeSinceLastRightGrab + (LeapGetNow() - LastLeapTime);
    }
    for (auto &Hand : CurrentFrame.Hands) {
      const FLeapHandData &FinalHandData = Hand;
      if (Hand.HandType == EHandType::LEAP_HAND_LEFT) {
    ...
    for (auto &Hand : CurrentFrame.Hands) {
      ...
      if (Hand.GrabStrength > StartGrabThreshold &&
          PastHand.GrabStrength <= StartGrabThreshold) {
        if (Hand.HandType == EHandType::LEAP_HAND_LEFT) {
          EmitKeyDownEventForKey(EKeysLeap::LeapGrabL);
        } else if (Hand.HandType == EHandType::LEAP_HAND_RIGHT) {
          EmitKeyDownEventForKey(EKeysLeap::LeapGrabR);
      ...
      // Release
      else if (Hand.GrabStrength <= EndGrabThreshold &&
               PastHand.GrabStrength > EndGrabThreshold) {
        if (Hand.HandType == EHandType::LEAP_HAND_LEFT) {
          EmitKeyUpEventForKey(EKeysLeap::LeapGrabL);
        } else if (Hand.HandType == EHandType::LEAP_HAND_RIGHT) {
          EmitKeyUpEventForKey(EKeysLeap::LeapGrabR);
...
void FUltraleapTrackingInputDevice::SetMessageHandler(
    const TSharedRef<FGenericApplicationMessageHandler> &InMessageHandler) {
  MessageHandler = InMessageHandler;
}

bool FUltraleapTrackingInputDevice::Exec(UWorld *InWorld, const TCHAR *Cmd,
                                         FOutputDevice &Ar) {
  // Nothing necessary to do (boilerplate code to complete the interface)
  return false;
}

void FUltraleapTrackingInputDevice::SetChannelValue(
    int32 ControllerId, FForceFeedbackChannelType ChannelType, float Value) {
  // Nothing necessary to do (boilerplate code to complete the interface)
}

void FUltraleapTrackingInputDevice::SetChannelValues(
    int32 ControllerId, const FForceFeedbackValues &values) {
  // Nothing necessary to do (boilerplate code to complete the interface)
}
...
void FUltraleapTrackingInputDevice::AddEventDelegate(
    const ULeapComponent *EventDelegate) {
  ...
  if (ComponentWorld->WorldType == EWorldType::Game ||
    ...
    UE_LOG(UltraleapTrackingLog, Log, TEXT("AddEventDelegate (%d)."),
           EventDelegates.Num());
  }
}

void FUltraleapTrackingInputDevice::RemoveEventDelegate(
    const ULeapComponent *EventDelegate) {
...
void FUltraleapTrackingInputDevice::AreHandsVisible(bool &LeftHandIsVisible,
                                                    bool &RightHandIsVisible) {
...
// Policies
void FUltraleapTrackingInputDevice::SetLeapPolicy(ELeapPolicyFlag Flag,
                                                  bool Enable) {
  switch (Flag) {
...
void FUltraleapTrackingInputDevice::UpdateInput(
    int32 DeviceID, class UBodyStateSkeleton *Skeleton) {
...
void FUltraleapTrackingInputDevice::OnDeviceDetach() {
  ...
  UE_LOG(UltraleapTrackingLog, Log,
         TEXT("OnDeviceDetach call from BodyState."));
}

void FUltraleapTrackingInputDevice::SetBSFingerFromLeapDigit(
    UBodyStateFinger *Finger, const FLeapDigitData &LeapDigit) {
...
void FUltraleapTrackingInputDevice::SetBSThumbFromLeapThumb(
    UBodyStateFinger *Finger, const FLeapDigitData &LeapDigit) {
...
void FUltraleapTrackingInputDevice::SetBSHandFromLeapHand(
    UBodyStateHand *Hand, const FLeapHandData &LeapHand) {
...
void FUltraleapTrackingInputDevice::SetOptions(const FLeapOptions &InOptions) {
  ...
  if (InOptions.TrackingFidelity == ELeapTrackingFidelity::LEAP_CUSTOM) {
    ...
    // Rift, note requires negative timewarp!
      ...
      if (InOptions.HMDRotationOffset.IsNearlyZero()) {
        Options.HMDRotationOffset =
            FRotator(-4, 0, 0); // typically oculus mounts sag a tiny bit
      }
  ...
  FLeapUtility::SetLeapGlobalOffsets(Options.HMDPositionOffset,
                                     Options.HMDRotationOffset);

  /*UseTimeBasedVisibilityCheck =*/UseTimeBasedGestureCheck =
      !Options.bUseFrameBasedGestureDetection;
...
FLeapOptions FUltraleapTrackingInputDevice::GetOptions() { return Options; }

FLeapStats FUltraleapTrackingInputDevice::GetStats() { return Stats; }
#pragma endregion Leap Input Device
