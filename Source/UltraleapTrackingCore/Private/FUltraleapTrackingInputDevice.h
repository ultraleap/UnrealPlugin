...
public:
   ...
   */
  virtual bool Exec(UWorld *InWorld, const TCHAR *Cmd,
                    FOutputDevice &Ar) override;

  /** IForceFeedbackSystem pass through functions **/
  virtual void SetChannelValue(int32 ControllerId,
                               FForceFeedbackChannelType ChannelType,
                               float Value) override;
  virtual void SetChannelValues(int32 ControllerId,
                                const FForceFeedbackValues &values) override;
  ...
  // BodyState
  virtual void UpdateInput(int32 DeviceID,
                           class UBodyStateSkeleton *Skeleton) override;
...
private:
  ...
  void CallFunctionOnComponents(
      TFunction<void(ULeapComponent *)>
          InFunction); // lambda multi-cast convenience wrapper
  ...
  void OnImageCallback(UTexture2D *LeftCapturedTexture,
                       UTexture2D *RightCapturedTexture);
  ...
  virtual void OnDeviceFailure(const eLeapDeviceStatus failure_code,
  ...
  virtual void
  OnTrackingMode(const eLeapTrackingMode current_tracking_mode) override;
  virtual void OnLog(const eLeapLogSeverity severity, const int64_t timestamp,
  ...
  void SetBSFingerFromLeapDigit(class UBodyStateFinger *Finger,
                                const FLeapDigitData &LeapDigit);
  void SetBSThumbFromLeapThumb(class UBodyStateFinger *Finger,
                               const FLeapDigitData &LeapDigit);
  void SetBSHandFromLeapHand(class UBodyStateHand *Hand,
                             const FLeapHandData &LeapHand);
};